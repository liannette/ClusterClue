...
def process_gbks(
    ...
    Args:
        ...
        gbks_file (str): Path to a file to store the paths to the input GenBank files.
    ...
    gbk_file_paths = list(iglob(os.path.join(gbks_dir_path, "*.gbk")))
    with open(gbks_file, "w") as f:
        f.write("\n".join(sorted(gbk_file_paths)))

    # Remove fasta files of do not have a gbk file
    cluster_ids = [
