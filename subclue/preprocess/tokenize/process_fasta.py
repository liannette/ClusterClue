...
def run_hmmscan(fasta_file_path, hmm_file_path, out_folder, verbose):
    ...
    Returns:
        str: Status of the operation, either "failed", "existed", or "converted".
    """
    ...
    if not os.path.isfile(fasta_file_path):
        print(f"Error running hmmscan on {fasta_file_path}: File doesn't exist")
        return "failed"
    ...
    # run hmmscan 
    # --cpu 0 to turn off multithreading
    log_file_path = os.devnull
    cmd = ["hmmscan", "-o", log_file_path, "--cpu", "0", "--domtblout", out_file_path, "--cut_tc", hmm_file_path, fasta_file_path]
    try:
